...
# load driving behaviour data, and return the dataset of sample list
# parameter description:
# filename: input driving behavior data file 
...
# steering_correction:  steering correction coefficient when using multiple camera's images
    ...
    while 1:  # Loop forever so the generator never terminates
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            for batch_sample in batch_samples:
                ...
                center_image_name = os.path.join("./data/IMG/", os.path.split(batch_sample[0])[-1])
                ...
                # Left Camera
                left_image_name = os.path.join("./data/IMG/", os.path.split(batch_sample[1])[-1])
                left_image = mpimg.imread(left_image_name)
                left_angle = center_angle + steering_correction
                images.append(left_image)
                angles.append(left_angle)
                # Right Camera
                right_image_name = os.path.join("./data/IMG/", os.path.split(batch_sample[2])[-1])
...
def main():
    ...
    # human drivers' behavior data file created from both track1 and track2
    filename = "./data/driving_log.csv"

    # load drivers' behavior data
    data_samples = load_data(filename)
